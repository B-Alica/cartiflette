...
def manual_tracker(use_cache=False):
    ...
    def get_living_area_insee():
        ...
        s = requests_cache.CachedSession("demoday_manual", expire_after=3600 * 24 * 2)
        s.proxies.update(proxies)
        url = (
            "https://www.insee.fr/fr/statistiques/fichier/6676988/fonds_bv2022_2022.zip"
        )
        r = s.get(url)
        ...
        r = s.get(url)
        o = io.BytesIO(r.content)
        pop = pd.read_excel(o, sheet_name="2020", skiprows=[0, 1, 2, 3, 4, 5, 6])
        ...
        pop2["codgeo"] = np.nan
        pop2[["codgeo", "Commune"]] = pop2.Commune.str.split(" - ", expand=True)
